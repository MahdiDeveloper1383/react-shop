...
const Home = () => {
  ...
  return (
    <React.Fragment>
      ...
      <Header />
      <Slider />
      <div className="flex flex-row lg:flex-row justify-between w-full mt-5">
        <Filters onChange={setFilter} />

        <div className="flex flex-row  flex-wrap  ml-auto mr-auto p-5 gap-5 justify-center  items-center sm:flex-row sm:gap-3 md:flex-row md:gap-5">
          <div className="flex flex-col sm:flex-row sm:flex-wrap  gap-5">
            {isLoading && (
              <h3 className="text-9xl font-bold text-yellow-400">loading...</h3>
            )}
          ...
          <div className="flex justify-center gap-2 mt-4">
            {Array.from(
              { length: Math.ceil(filteredBooks.length / ItemPerPage) },
              (_, i) => (
                <button
                  key={i}
